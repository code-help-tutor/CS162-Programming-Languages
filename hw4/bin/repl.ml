  ...
  | CLet (x, t, def) ->
      let def_v' =
        if !Opts.meta then (
          ...
          let def_e = context_expr def in
          (if !Opts.typed then
             let ty = Typecheck.abstract_eval [] def_e in
             Fmt.pr "<-- type: %a\n%!" Pretty.pp_ty ty);
          let def_v = get_eval () def_e in
          Fmt.pr "%s = %a\n%!" x Pretty.pp_expr def_v;
          def_v
      in
      get_context () := ((x, t), def_v') :: !(get_context ())
  | CEval e ->
      Fmt.pr "<== %a\n%!" Pretty.pp_expr e;
