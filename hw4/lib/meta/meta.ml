...
let rec fold (f : expr -> expr) (e : expr) : expr =
  match e with
  | Var _ | Num _ | True | False | ListNil _ -> f e
  | Lambda (t, Scope (x, e)) -> f (Lambda (t, Scope (x, fold f e)))
  ...
  | Fix (t, Scope (x, e)) -> f (Fix (t, Scope (x, fold f e)))
        ...
        App
          ( App (e1, e2),
            Lambda
              ( None,
                Scope
                  (x, Lambda (None, Scope (y, Lambda (None, Scope ("_", e3)))))
              ) )
    | e -> e

  let fix = function
    | Fix (_, Scope (x, e)) -> App (get "fix_z", Lambda (None, Scope (x, e)))
    | e -> e
...
let normalize (e : expr) : expr =
  ...
  let rec helper env e =
    ...
    | Lambda (t, Scope (x, e')) ->
        let n = fresh_id () in
        Lambda (t, Scope (Int.to_string n, helper ((x, n) :: env) e'))
    | App (f, a) ->
        let f' = helper env f in
        let a' = helper env a in
        App (f', a')
    | Let (e1, Scope (x, e2)) ->
        helper env (App (Lambda (None, Scope (x, e2)), e1))
    | _ ->
        raise
...
let rec prepend_name (x : string) (e : expr) : expr =
  match e with
  | Var y -> Var (x ^ y)
  | Lambda (_, Scope (y, e)) -> Lambda (None, Scope (x ^ y, prepend_name x e))
